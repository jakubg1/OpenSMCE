...
def docld_to_lua(entry, class_name, schema_path, is_root = True, omit_packing = False, context = "", data_context = None, error_context = None, iterators_used = 0):
	...
	if is_root and not omit_packing:
		...
		out.append("")
		out.append("---Retrieves a " + class_name + " by a given path or alias.")
		out.append("---@param reference string|integer The path or an alias to the resource.")
		out.append("---@param skipAliasResolutionCheck boolean? If set, the resource will be returned even if it has unresolved alias references. You should only set this to `true` if you do not intend to interact with the config's contents.")
