...
function Highscores:storeEntry(position, name, score, level)
	...
	self.entries[position] = {name = name, score = score, level = level}
end

---Returns highscores' save data.
---@return table
function Highscores:serialize()
	return {entries = self.entries}
end

---Loads highscores from previously saved data.
---@param t table Highscore save data.
function Highscores:deserialize(t)
