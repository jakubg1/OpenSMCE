...
function ProfileSession:getMapData()
	return self.levelID and _Game.game:getMapData(self.levelID.map)
end
...
function ProfileSession:writeHighscore()
	local pos = _Game.game.highscores:getPosition(self.score)
	if not pos then
		return
	end
	_Game.game.highscores:storeEntry(pos, self.profile.name, self.score, self:getLevelName())
	return pos
end
