...
---Represents a Level. Houses the Map, Shooters, Shot Spheres, Collectibles and Floating Texts.
---Handles elements such as level objectives and general level event order.
---@class Level
...
function Level:new(config)
	...
	if _Game.satMode then
		objectives = {{type = "destroyedSpheres", target = self:getSession():getUSMNumber() * 10}}
	end
	---@alias LevelObjective {type: string, target: number, progress: number, reached: boolean}
	---@type LevelObjective[]
	self.objectives = {}
	...
	self.startingTimers = _Game.game.gameplayConfig.levelTimers
	self.startingTimerSeries = _Game.game.gameplayConfig.levelTimerSeries
	self.scoreFormat = _Game.game.gameplayConfig.scoreFormat
...
function Level:updateMusic()
	...
	if not mute and not danger then
		music:getTrack():play(1)
	else
		music:getTrack():pause(1)
	end
	if dangerMusic then
		if not mute and danger then
			dangerMusic:getTrack():play(1)
		elseif self.pause then
			dangerMusic:getTrack():pause(1)
		else
			dangerMusic:getTrack():stop(1)
		end
	end
	...
	if _DFLAG_ASL then
		if self.danger then
			music:getTrack():setSpeed(3, 1)
		else
			music:getTrack():setSpeed(1, 1)
		end
	end
...
function Level:updateMisc(dt)
	...
	if _Game.game.gameplayConfig.sphereBehavior.cascadeScope == "level" and not self:isMatchPredicted() then
		self:endCascade()
	end
end
	...
	elseif step.type == "fail" then
		if self.failDestructionDelay then
			...
			if self.failDestructionDelay <= 0 then
				...
				local failConfig = _Game.game.gameplayConfig.sphereBehavior.foulDestroySpheres
...
---Returns the current player's session during which this Level is being played.
---@return ProfileSession
function Level:getSession()
	return assert(_Game.game:getSession(), "Level cannot exist when Session is not active!")
end
...
function Level:evaluateCollectibleGeneratorEntry(generator)
	...
	_Vars:set("generator.latestCheckpoint", self:getSession():getLatestCheckpoint())
...
function Level:executeScoreEvent(scoreEvent, x, y)
	...
	if _Game.game:getProfile().ultimatelySatisfyingMode then
		score = math.floor(score * (1 + (self:getSession():getUSMNumber() - 1) * 0.2) + 0.5)
	end
	local unmultipliedScore = score
	if not scoreEvent.ignoreDifficultyMultiplier then
		score = score * self:getSession():getDifficultyConfig().scoreMultiplier
	end
...
function Level:hasNewScoreRecord()
	return self:getSession():getLevelHighscore(self.score)
end
...
function Level:restartMusic()
	self.config.music:nextTrack()
	if not self:isMusicMuted() then
		self.config.music:getTrack():play()
	end
	if _DFLAG_ASL then
		self.config.music:getTrack():setSpeed(1)
	end
end
...
function Level:destroy()
	...
	if self.config.music then
		self.config.music:getTrack():stop(1)
	end
	if self.config.dangerMusic then
		self.config.dangerMusic:getTrack():stop(1)
	end
...
function Level:reset()
	...
	self.variables = {}
	if _Game.game.gameplayConfig.levelVariables then
		for variable, value in pairs(_Game.game.gameplayConfig.levelVariables) do
...
function Level:spawnNet()
	local netConfig = _Game.game.gameplayConfig.net
	local w, h = _Game:getNativeResolution()
