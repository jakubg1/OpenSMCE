...
function NetworkingTest:new()
    ...
    self.timeoutThreshold = 15 -- After how many seconds should the client be informed that the server is not responding and to be disconnected, or the host remove the user from the list.
end
...
function NetworkingTest:update(dt)
    ...
    if self.socket then
        if self.isHost then
            -- Check if any person has not been online for more than the allowed threshold.
            local names = self.userList:getDeadUserNames(self.timeoutThreshold)
            for i, name in ipairs(names) do
                -- Kick dead users.
                self.userList:removeUser(name)
                self:broadcastError(string.format("User %s timed out.", name))
            end
        else
            if not self.lastPingTime then
            ...
            elseif _GetPreciseTime() - self.lastPingTime >= self.timeoutThreshold then
                self:disconnect()
                self:printError("Connection lost: Timed out!")
            end
        end
    end
end
...
function NetworkingTest:list()
    ...
    if self.isHost then
        ...
        for i, name in ipairs(self.userList:getSortedUserNames()) do
            ...
            s = s .. (user.isHost and "----" or string.format("%d", self.timeoutThreshold - (_GetPreciseTime() - user.atime)))
...
---Broadcasts an error message to all connected clients if you're a host.
---@private
---@param message string The error message.
function NetworkingTest:broadcastError(message)
    assert(self.isHost, "Clients cannot broadcast messages! Are you stupid?")
    self:printError(message)
    self:sendPacket({type = "ERR", message = message})
end
...
function NetworkingTest:receivePacket(payload, ip, port)
    if self.isHost then
        ...
        elseif payload.type == "PING" then
            ...
            self.userList:markUserHeartbeat(name)
